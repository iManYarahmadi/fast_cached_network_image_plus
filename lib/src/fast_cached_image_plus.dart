...
class FastCachedImagePlus extends StatefulWidget {
  ...
  ///[FastCachedImagePlus] creates a widget to display network images. This widget downloads the network image
...
class _FastCachedImagePlusState extends State<FastCachedImagePlus>
...
///[FastCachedImagePlusConfig] is the class to manage and set the cache configurations.
class FastCachedImagePlusConfig {
  ...
  static Future<Uint8List?> _getImage(
    ...
    if (_imageKeyBox!.keys.contains(identifier) &&
      ...
      await _replaceImageKey(oldKey: identifier!, newKey: key);
      await _replaceOldImage(
          oldKey: identifier,
          newKey: key,
          image: await _imageBox!.get(identifier));
    }
  ...
  static Future<void> clearAllCachedImages({bool showLog = true}) async {
    _checkInit();
    // First, ensure that the boxes are properly closed
    await _imageBox!.close();
    await _imageKeyBox!.close();
    final futures = [
...
// Wait for all futures to complete
    ...
    // Create a list of futures to reopen the boxes
    final reopeningFutures = [
      Hive.openLazyBox(_BoxNames.imagesKeyBox),
      Hive.openLazyBox(_BoxNames.imagesBox),
    ];

    // Wait for all reopening futures to complete
    await Future.wait(reopeningFutures).then(
      (value) {
        // Assign reopened boxes
        _imageKeyBox = value[0];
        _imageBox = value[1];
      },
    );
  ...
  static void _checkInit() {
    if ((FastCachedImagePlusConfig._imageKeyBox == null ||
            !FastCachedImagePlusConfig._imageKeyBox!.isOpen) ||
        FastCachedImagePlusConfig._imageBox == null ||
        !FastCachedImagePlusConfig._imageBox!.isOpen) {
...
@immutable
class FastCachedImagePlusProvider extends ImageProvider<NetworkImage>
  ...
  Future<ui.Codec> _loadAsync(
    FastCachedImagePlusProvider key,
  ...
  bool operator ==(Object other) {
    ...
    return other is FastCachedImagePlusProvider &&
        other.url == url &&
        other.scale == scale;
  }
